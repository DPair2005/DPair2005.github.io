...
update: 特别鸣谢 **八宝狮子头** 对本文提出的建议。
## 0 前期准备

### 0.1 游戏方面
- PC端游戏下载 https://adl.netease.com/d/g/id5/c/gw?type=PC%E2%80%8B
- PC端免扫码登录工具 https://wiki.biligame.com/dwrg/PC%E7%AB%AF%E5%85%8D%E6%89%AB%E7%A0%81%E7%99%BB%E5%BD%95%E5%B7%A5%E5%85%B7

**另外请确保导播拥有两个及以上可以登录 PC 端第五人格的账号。若无则应当向其他导播借用。**
...
- 安装 bp 软件（通过联系主办方获得）
- 安装所需字体（安装方式为将所有字体.ttf 文件复制到 `C:\Windows\fonts` 下，所有字体文件通过主办方获得）

~~由于撰写本文时笔者没有安装字体，所以部分图片中字体事实上是不对的，还请忽略这个小问题~~
...
- 值得注意的是，正式比赛中应当设置 **直播延迟**，具体方式为在直播姬中选择 `直播设置-推流延迟`，然后将延迟设置为 60s
...
**裁判** 作为自定义房间的房主开启自定义，确认4求生+1监管的位置都已经调整好，确认观战席2导播+2解说+1裁判（战队成员不允许进入观战席）。
...
**update: 值得一提的是，经过实测，现在 bilibili 网页端也用有分P上传功能可以直接使用。若此法行不通再参考下文。**

可参考：[【实用教程】在B站如何上传分P视频稿件详细教程】](https://www.bilibili.com/video/BV1G84y1T7qB/?share_source=copy_web&vd_source=1eb4b1502b550f5b41fc18869b271432)

推荐使用biliup-app上传，可扫码登录账号，后续无需再次登录。哔哩哔哩投稿工具仅支持短信验证码登录，且后续需要再次登录。

初次下载哔哩哔哩投稿工具可能遇见登录界面明显偏小、图片滑块验证显示不全的问题。解决方法：选中软件后右键，选择【属性】，上方选项卡【兼容性】，选择【更改高DPI设置】，勾选【替代高DPI缩放行为】，点击【确定】-【应用】，重新打开软件。
...
有些时候我们可能需要修改 **非正在被推送的其他场景** 中的内容，即我们在推送场景 1 时可能需要悄悄修改场景 2 的内容。此时选择右侧的 `工作室模式`，即可发现更改的场景和输出的场景被区分开来了：

![](./bonus8.png)

如图，此时我们输出的是区域选择的场景，但是修改的是正式对局的场景。
#### 3.1.3 其他细节补充

使用OBS录制前确认视频保存地址有足够的空间储存视频，否则在直播中会出现录制中断的情况。更改视频保存地址的方法：【设置】-【输出】-【录像】-【录像路径】

使用bp系统后台输入信息但前台界面没有相应更新，是出现了脱机问题。解决方法：拖动前台界面窗口，让没有更新新信息的位置在电脑屏幕中显示。

第五人格游戏内【设置】-【画质】-【战斗内帧率检测】选择“关”，避免局内弹窗提醒。
