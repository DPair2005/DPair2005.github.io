...
~~这个憨批 DPair 为了抢一血写了一个下午的根号做法然后卡了一个晚上的常，最终在 **神·zhoukangyang** 秒掉这道题后才觉悟过来有超级 naive 的 polylog 做法~~
...
因此能和一个点形成一个 **计入贡献的合法点对** 必然它和它在其他子树内的前驱后继

考虑我们在更前面的点处产生贡献，那么就是它在其他子树内的前驱

那么就转化为了一个经典的问题，可以用 dsu on tree 解决

然后发现复杂度也一并解决了，因为每一个点只会求一次，因此这样的点数就是 $O(n\log n)$ 的

因此总工是 $O(n\log n)$ 次单点修改和前驱后继操作，然后是 $O(q)$ 次区间修改
